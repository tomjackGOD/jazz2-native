...
namespace nCine
{
	...
	void BaseSprite::shaderHasChanged()
	{
		...
		BackendUniformCache* textureUniform = renderCommand_.GetMaterial().Uniform(Material::TextureUniformName);
	...
	void BaseSprite::updateRenderCommand()
	{
		...
		if (dirtyBits_.test(DirtyBitPositions::ColorUploadBit)) {
			BackendUniformCache* colorUniform = instanceBlock_->GetUniform(Material::ColorUniformName);
		...
		if (dirtyBits_.test(DirtyBitPositions::SizeBit)) {
			BackendUniformCache* spriteSizeUniform = instanceBlock_->GetUniform(Material::SpriteSizeUniformName);
		...
		if (dirtyBits_.test(DirtyBitPositions::TextureBit)) {
			if (texture_ != nullptr) {
				...
				BackendUniformCache* texRectUniform = instanceBlock_->GetUniform(Material::TexRectUniformName);
